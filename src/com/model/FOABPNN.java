...
public class FOABPNN {
    ...
    private void RandomWeight(double[] x, double[] y) {
        for(int n = 0; n < hiddenNum ; n++) {
            for(int m = 0; m < inputNum; m++) {
                w[m][n] = sqr(x[m * hiddenNum + n], y[m * hiddenNum + n]);
            }
            hidden_y[n] = sqr(x[hiddenNum * inputNum + n], y[hiddenNum * inputNum + n]);
        }

        int base = inputNum * hiddenNum + hiddenNum + outputNum;
        for(int n = 0; n < outputNum ; n++) {
            for(int m = 0; m < hiddenNum; m++) {
                v[m][n] = sqr(x[base + m * outputNum + n], y[base + m * outputNum + n]);
            }
            out_y[n] = sqr(x[inputNum * hiddenNum + hiddenNum + n], y[inputNum * hiddenNum + hiddenNum + n]);
        }
    }

    private double sqr (double x, double y) {
        double sum = Math.pow(x, 2) + Math.pow(y, 2);
        return 1 / Math.pow(sum, 0.5);
    }
